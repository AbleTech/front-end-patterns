  ...
  function Tabs(){
    ...
    function _tabClickHandler(event){
      ...
      if (nodeType != 'A') {
        ...
        var targetIndex = parseInt(elem.getAttribute(ANCHOR_DATA_ATTRIBUTE));
        var tabContainer = utils.findMatchingParentBySelector(elem, '.' + CLASS_MATCH);
        if (tabContainer) {
          tabContainer.setAttribute(DATA_ATTRIBUTE, targetIndex);
        }
      }
    }
    function _addAnchorIndexes(arrayOfLinks){
      for (var i = 0; i < arrayOfLinks.length; i++) {
        ...
        anchor.setAttribute(ANCHOR_DATA_ATTRIBUTE, i+1);
      }
    }
    /* *******************************************************/
    ...
    t.findMatchingElements = function(){
      t.tabs = d.querySelectorAll('.' + CLASS_MATCH);
    }
    t.initialiseTabGroup = function(){
      for (var i = 0; i < t.tabs.length; i++) {
        var currentTabGroup = t.tabs[i];
        if (currentTabGroup.getAttribute(DATA_ATTRIBUTE)) {
          continue;
        } else {
          var initialIndex = currentTabGroup.getAttribute(INITIAL_DATA_ATTRIBUTE) || 1;
          currentTabGroup.setAttribute(DATA_ATTRIBUTE, initialIndex);
