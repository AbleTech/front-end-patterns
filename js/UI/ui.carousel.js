...
(function(w, d){
  ...
  function Carousel(elem){
    ...
    function _handleNavClick(event){
      ...
      if (clickedNode.nodeName === 'A') {
        if (clickedNode.classList.contains(NAV_CLASS_NEXT)) {
          c.setSlideIndex(1);
        }
        if (clickedNode.classList.contains(NAV_CLASS_PREVIOUS)) {
          c.setSlideIndex(-1);
        }
      }
    }
    ...
    var c = this;
    c.elem = elem;
    c.container = c.elem.querySelector('.' + CONTAINER_CLASS);
    ...
    c.setSlideIndex = function(increment) {
      ...
      c.container.setAttribute(DATA_ATTR_INDEX, c.currentIndex);
    }
    c.countSlides = function(){
      var slides = c.container.querySelectorAll(SELECTOR_SLIDE);
      c.container.setAttribute(DATA_ATTR_COUNT, slides.length);
    ...
    c.setupCarousel = function(){
      c.maxSlides = c.container.getAttribute(DATA_ATTR_COUNT) || c.countSlides();
      c.currentIndex = c.elem.getAttribute(DATA_ATTR_INDEX) || 0;
      c.bindLinks();
    }
    c.init = function(){
      if (c.elem) {
  ...
  function Carousels(){
    ...
    cs.init = function(){
      cs.listOfCarousels = d.querySelectorAll('.' + PANEL_CLASS);
