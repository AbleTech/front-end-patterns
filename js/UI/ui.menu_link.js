...
(function(w, d, UI){
  ...
  function MenuLink(){
    ...
    /* Private **********************************************/
    function _handleMaskClick(){
      ml.closeMenu();
    }
    function _handleMenuLinkClick(event){
      event.preventDefault();
      (d.querySelector('body.' + MENU_CLOSED_CLASS)) ? ml.openMenu() : ml.closeMenu();
    }
    ...
    ml.closeMenu = function(){
      d.body.classList.add(MENU_CLOSED_CLASS);
      w.removeEventListener('keyup', _trapKeyUp);
    }
    ml.openMenu = function(){
      d.body.classList.remove(MENU_CLOSED_CLASS);
    ...
    ml.addMask = function(){
      ...
      mask.id = MENU_MASK_ID;
      mask.className = MENU_MASK_CLASS;
      d.body.appendChild(mask);
      ml.mask = d.getElementById(MENU_MASK_ID);
    }
    ml.init = function(){
      ml.menu      = d.getElementById(MENU_CONTAINER_ID);
      ml.menu_link = d.getElementById(MENU_LINK_ID);
      ml.mask      = d.getElementById(MENU_MASK_ID);
